...
class CUUID {
  public:
    ...
    friend bool operator==(const CUUID &lhs, const CUUID &rhs) {
    ...
    friend bool operator!=(const CUUID &lhs, const CUUID &rhs) { return !(lhs == rhs); }
    friend bool operator> (const CUUID &lhs, const CUUID &rhs) { return rhs < lhs; }
    friend bool operator<=(const CUUID &lhs, const CUUID &rhs) { return !(lhs > rhs); }
    friend bool operator>=(const CUUID &lhs, const CUUID &rhs) { return !(lhs < rhs); }
    ...
    friend std::ostream& operator<< (std::ostream& stream, const CUUID& uuid) {
      return stream << uuid.str();
    }

    friend std::istream& operator>> (std::istream& stream, CUUID& uuid) {
      std::string s;
      stream >> s;
...
class CUUIDGenerator {
  public:
    CUUIDGenerator() : generator(new RNG(std::random_device()())), distribution(std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max())
    {}

    CUUIDGenerator(uint64_t seed) : generator(new RNG(seed)), distribution(std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max())
    {}

    CUUIDGenerator(RNG &gen) : generator(gen), distribution(std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max())
    {}
...
namespace std {
  template <> struct hash<CUUID>
  {
    size_t operator()(const CUUID & uuid) const
    {
      return uuid.hash();
    }
  };
